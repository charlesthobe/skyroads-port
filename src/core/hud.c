                 ...
                 sr_play* p, const uint32_t tick)
{
  ...
  for (int i = 0; i < 34; i++)
  {
    ...
    if (!speed_rec(a, i, &r))
      break;
    draw_cells(fb, pristine, r.ofs, r.w, r.h, r.cells, 0x5e, 0x5f, i < sp_segs);
  }
  ...
  {
    // Apparently sr_aplight[0] just says "IDLE" which is the same as the
    // "pristine" HUD, nonetheless we let it be used for now at least.
  ...
  // GRAV-O-METER: 1 digit at (0x65 + 5,0x9c), (value = gravity - 3), and 2
  // prerendered zeros from the HUD. Technically can handle 2 digits although
  // this never happens in the original game.
  {
